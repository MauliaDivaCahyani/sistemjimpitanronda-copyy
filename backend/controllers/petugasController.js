...
export const createPetugas = async (req, res) => {
  try {
    ...
    const [result] = await pool.query(
      ...
      [idWarga, jabatan || null, role || "petugas", status, username, password, idKelompokRonda || null],
    )
...
export const updatePetugas = async (req, res) => {
  try {
    ...
    console.log("DEBUG UPDATE PETUGAS - Data received:", { id, jabatan, role, statusUser, username, idKelompokRonda })
    ...
    const queryParams = [jabatan || null, role || "petugas", status, username, idKelompokRonda || null]
